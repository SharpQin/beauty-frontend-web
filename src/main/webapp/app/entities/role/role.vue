<template>
  <div>
    <h2 id="page-heading" data-cy="RoleHeading">
      <span v-text="$t('beautyApp.role.home.title')" id="role-heading">Roles</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('beautyApp.role.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RoleCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-role">
            ...
            <span v-text="$t('beautyApp.role.home.createLabel')"> Create a new Role </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && roles && roles.length === 0">
      <span v-text="$t('beautyApp.role.home.notFound')">No roles found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('beautyApp.role.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('dsc')">
              <span v-text="$t('beautyApp.role.dsc')">Dsc</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="beautyApp.role.delete.question" data-cy="roleDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="au-delete-role-heading" v-text="$t('beautyApp.role.delete.question', { id: removeId })">
